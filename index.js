  ...
  $("form").submit(event => {
    ...
    function getLatLngByZipcode(latitude, longitude) {
        ...
        function catCall(latitude, longitude) {
          ...
          let catApiLinkDS = darkSkyURL;
        
          $.ajax({
            url: Proxy + catApiLinkDS,
            success:function(data) { 
              let catSummary = "The current condition is " + data.currently.summary.toLowerCase() + ".";
              let catTemp = "The current temperature is " + data.currently.temperature + " degrees F."; 
              let catApparent = "It feels like " + data.currently.apparentTemperature + " degrees F.";
          ...
          let dogProxy = "https://cors-anywhere.herokuapp.com/";
          let dogApiLocation = accuWeatherBaseLocation + fromZipInput + ",us&apikey=" + AccuWeatherAPI;

          $.ajax({
            url: Proxy + dogApiLocation,
            success:function(data) { 
              let dogLocation = data[0].Key;
              let dogApiLinkDS = accuWeatherBase + dogLocation + "?apikey=" + AccuWeatherAPI + "&language=en-us&details=false HTTP/1.1";
          ...
          function secondDogCall(dogApiLinkDS) {
            $.ajax({
              ...
              success:function(data) { 
                let dogSummary = "The current condition is " + data[0].WeatherText.toLowerCase() + ".";
                let dogTemp = "The current temperature is " + data[0].Temperature.Imperial.Value;
                let dogTempUnit = data[0].Temperature.Imperial.Unit + ".";
          ...
          let weatherBitURL = weatherBitBase + "postal_code=" + fromZipInput + "&country=US&units=I&key=" + weatherBitAPI; 
          ...
          .then((myJson => {
            let birdSummary = "The current condition is " + myJson.data["0"].weather.description.toLowerCase() + ".";
            let birdTemp = "The current temperature is " + myJson.data["0"].temp + " degrees F.";
            let birdApparent = "It feels like " + myJson.data["0"].app_temp + " degrees F.";
